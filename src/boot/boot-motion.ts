...
export function bootMotion(appTime: number) {
      ...
      : t < 11.12
        ...
        : t < 19.48
          ...
          : t < 22.76
            ? "scan"
            : "welcome";
  let auth = typed("START PROCESSING", f, 282, 300);
  if (f >= 305) auth += ".".repeat(1 + Math.floor((f - 305) / 12) % 3);
  ...
  return {
    ...
    logoLetters: typed("DataRocks", f, 232, 255),
    ...
    poweredLetters: typed("POWERED BY DataRocks", f, 279, 295).length,
