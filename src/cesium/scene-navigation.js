import { CHINA_VIEW } from './locations.js';
export function createSceneNavigation(viewer, onPhase) {
  ...
  function navigate(mine, terrainHeight = 0, instant = false) {
    ...
    if (!mine) {
      const {longitude,latitude,height,heading,pitch,roll} = CHINA_VIEW;
      ...
      const center = C.Cartesian3.fromDegrees(yulong ? 97.7353 : mine.longitude, yulong ? 31.4125 : mine.latitude, terrainHeight);
      const width = viewer.scene.canvas.clientWidth || 1280;
      const safeFraction = Math.max(.32, (width - (width < 1100 ? 360 : 710)) / width);
      const radius = mine.sceneRadius ?? (yulong ? 310 : 2200);
  ...
  function focusEquipment(position) {
    ...
    viewer.camera.flyToBoundingSphere(new C.BoundingSphere(center, 15), {
      offset: new C.HeadingPitchRange(0, C.Math.toRadians(-48), 150),
