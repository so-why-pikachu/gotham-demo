...
import {AS_OF} from './scenario.mjs';
...
export function mineMetrics(mineId, range) {
  ...
  const series = connected ? Array.from({ length: samples }, (_, i) => ({
    ...
    dust: Number((0.32 + 0.04 * Math.sin((i - samples + 1) * step/3600000 * 0.8)).toFixed(2)),
    workers: 126 + ((((i - samples + 1)*step/3600000) % 5 + 5) % 5) * 2,
  })) : [];
  return {
    ...
    groups: [
      { title: '设备与施工', items: [
        ...
        ['矿卡 / 挖掘机 / 装载机', connected ? ['XDE240','XE215C','XC958U'].map(model=>fleet.filter(e=>e.model===model).length).join(' / ') : null, '台'],
      ...
      { title: '人员与故障', items: [
        ...
        ['待复检工单', connected ? fleet.filter(e=>alerts(e.id).length).length : null, '张'],
      ] },
    ],
  };
}
